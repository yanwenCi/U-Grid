...
import numpy as np
import src.model.networks.configs_TransMorph as configs
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, pretrain_img_size=128,
        ...
        print('use checkpoints', use_checkpoint)
...
class TransMorph(nn.Module):
    ...
    def forward(self, x):
        ...
        #print(out_feats[0].shape, out_feats[1].shape, out_feats[2].shape, out_feats[-1].shape, x.shape, flow.shape)
...
class TransEncoder(nn.Module):
    def __init__(self, config, pretrained = False, adaption=True):
        '''
        TransMorph Model
        '''
        ...
        if pretrained:
            ...
            pretrained_model = torch.load(config.pretrained_path)['state_dict']
            pretrained_model = {k.replace('transformer.', ''): v for k, v in pretrained_model.items()}
          
            pretrained_model = {k: v for k, v in pretrained_model.items() if k in transformer_state}
            #print(pretrained_model.keys())
            transformer_state.update(pretrained_model)
        ...
        for i in range(len(config.depths)):
            # Example: adding adaptation layers to BERT encoder layers
            layer = self.transformer.layers[i]
            print(self.transformer.layers[i])
            # Assuming the hidden size and adaptation size are chosen
            hidden_size = layer.blocks[-1].mlp.fc2.out_features
            adaptation_size = 1024

            # Insert the adaptation layer after the self-attention mechanism
            adaptation_layer = AdaptationLayer(hidden_size, hidden_size, adaptation_size)
            layer.blocks[-1].add_module(f"adaptation_{i}", adaptation_layer)
    ...
    def forward(self, x):
        ...
        out_feats = self.transformer(x)
        #print('949', len(out_feats), out_feats[-1].shape)
        if self.if_transskip:
            f1 = out_feats[-2]
            f2 = out_feats[-3]
            f3 = out_feats[-4]
        ...
        # flow = self.reg_head(x)
        # out = self.spatial_trans(source, flow)
        # print(out_feats[-4].shape, out_feats[-3].shape, out_feats[-2].shape, out_feats[-1].shape)
        out_feats_last = self.avg_pool(out_feats[-1])
        return out_feats_last.view(out_feats[-1].shape[0], -1) #out, flow



class TransMorphTrainer(nn.Module):
    '''implicit correspondence network'''
    def __init__(self, CONFIGS):
        super(TransMorphTrainer, self).__init__()
        
        self.img_enc = TransMorph(CONFIGS['TransMorph'])
    
    def forward(self, x):
        '''
        grid --> [batch, 3, h, w, z]
        '''
        field = self.img_enc(x)
       
        return field  # Grid displacement field