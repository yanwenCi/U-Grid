...
from src.model.networks import TransMorph, VoxelMorph
...
class icReg(BaseArch):
        ...
        for self.epoch in range(1, self.config.num_epochs + 1):
            ...
            for self.step, input_dict in enumerate(self.train_loader):
                ...
                if self.model == 'TransMorph' or self.model == 'VoxelMorph':
                    ...
                    gdfs = self.net(torch.cat([fx_img, mv_img], dim=1))
                    grid = []
                    global_loss = 0
                    for gdf in gdfs:
                        grid=smfunctions.get_reference_grid3d(img=fx_img, grid_size=gdf.shape[-3:])
                        Gsample_fx_img = smfunctions.warp3d(fx_img, ddf=torch.zeros(grid.shape).cuda(), ref_grid=grid) 
                        Gsample_fx_seg = smfunctions.warp3d(fx_seg, ddf=torch.zeros(grid.shape).cuda(), ref_grid=grid)
                        Gsample_warpped_mv_img = smfunctions.warp3d(mv_img, ddf=gdf, ref_grid=grid)
                        Gsample_warpped_mv_seg = smfunctions.warp3d(mv_seg, ddf=gdf, ref_grid=grid)

                         # Calculate volumn-level images
                        ddf = F.interpolate(gdf, size=self.config.crop_size, mode='trilinear', align_corners=True)
    ...
    def validation(self):
        ...
        for idx, input_dict in enumerate(self.val_loader):
            ...
            if self.model == 'TransMorph' or self.model == 'VoxelMorph':
                # Calculate gird-level images
                gdf = self.net(torch.cat([fx_img, mv_img], dim=1))[1]
                grid = smfunctions.get_reference_grid3d(img=fx_img, grid_size=gdf.shape[-3:]) 
            elif self.model == 'ICNet_control':
                gdf, grid = self.net(torch.cat([fx_img, mv_img], dim=1))    
            else:
