...
from src.model.networks import TransMorph, VoxelMorph, icn_multi
...
class icReg(BaseArch):
    ...
    def net_parsing(self):
        ...
        elif self.model == 'SegReg':
            net = icn_multi.Segcorrection(self.config)
        else:
    ...
    def train(self):
        ...
        for self.epoch in range(1, self.config.num_epochs + 1):
            ...
            for self.step, input_dict in enumerate(self.train_loader):
                ...
                img = torch.cat([fx_img, mv_img], dim=0)
                label = torch.cat([fx_seg, mv_seg], dim=0)
                seg, gdf = self.net(img)
                
                grid = smfunctions.get_reference_grid3d(img=img, grid_size=gdf.shape[-3:]) 
    
                correct_seg = smfunctions.warp3d(seg, ddf=gdf, ref_grid=grid)

                #ddf = F.interpolate(gdf, size=self.config.input_shape, mode='trilinear', align_corners=True)
               
                
                # loss functions 
                seg_loss = loss.dice_loss(label, seg) * self.config.w_Gdsc
                correct_loss = 0#loss.dice_loss(label, correct_seg) * self.config.w_Idsc

                bending = loss.bending_energy(gdf) * self.config.w_bde  ##### might need change

                global_loss = seg_loss + correct_loss + bending
                global_loss.backward()

                print(f'L_All:{global_loss:.3f}, BDE: {bending:.6f}, correct: {correct_loss:.3f}, seg: {seg_loss:.3f}')
                optimizer.step()
    ...
    def validation(self):
        ...
        for idx, input_dict in enumerate(self.val_loader):
            ...
            correct_seg = smfunctions.warp3d(seg, gdf).argmax(dim=1, keepdim=True)
            seg_bin = seg.argmax(dim=1, keepdim=True)
    
            aft_dsc = loss.binary_dice(label.cpu().data, correct_seg.cpu().data)
            bef_dsc = loss.binary_dice(label.cpu().data, seg_bin.cpu().data)

            print(idx, f'mv:{mv_key}', f'fx:{fx_key}', f'Before-DICE:{bef_dsc:.3f}', f'After-DICE:{aft_dsc:.3f}')
            res.append(aft_dsc)
            bfr.append(bef_dsc)
        
        mean, std = torch.mean(torch.tensor(res)), torch.std(torch.tensor(res))
        bfr_mean = torch.mean(torch.tensor(bfr))
        ...
        differences = [m1 - m2 for m1, m2 in zip(res, bfr)]
        ...
        print('Before Dice:', bfr_mean, 'After Dice:', mean, std, 'Best After Dice:', self.best_metric, 'Escape Time:', escape_time)
