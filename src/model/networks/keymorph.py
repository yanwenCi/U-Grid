...
from src.model.networks.keypoint_aligners import RigidKeypointAligner, AffineKeypointAligner, TPS
from src.model.networks.keymorph_layers import (
    LinearRegressor2d,
    LinearRegressor3d,
    CenterOfMass2d,
    CenterOfMass3d,
)
from src.model.networks.keymorph_utils import str_or_float, rescale_intensity
from src.model.networks.keymorph_net import ConvNet, ImageEncoder
...
class KeyMorph(nn.Module):
    def __init__(
        ...
        if self.backbone == 'conv':
            self.net = ConvNet(dim, 1, 512, "instance", num_layers)
            #self.net = ImageEncoder(1)
        elif self.backbone == 'unet':
            self.net = Simple_Unet(1, num_keypoints, True, [32, 64, 64, 128], [128, 64, 64, 32])
        else: 
        ...
        if keypoint_layer == "com":
            ...
            if dim == 2:
                self.keypoint_layer = LinearRegressor2d(1000, self.num_keypoints )
            else:
                self.keypoint_layer = LinearRegressor3d(1000, self.num_keypoints)
    ...
    def get_keypoints(self, img, return_feat=False):
        ...
        # print(img.shape, feat.shape, points.shape)
        if return_feat:
            return points, feat
        return points
    ...
    def forward(self, img_f, img_m, transform_type="affine", **kwargs):
        ...
        return grid.permute(0, 4, 1, 2, 3)#result_dict
