...
def dice_loss(y_true, y_pred):
    ...
    y_true = torch.scatter(torch.zeros_like(y_pred), 1, y_true, 1)
    numerator = 2 * torch.sum(y_true * y_pred, axis=[ 2, 3, 4])
    denominator = torch.sum(y_true, axis=[2, 3, 4]) + torch.sum(y_pred, axis=[2, 3, 4])
    dice = torch.mean(numerator / denominator, axis=[0,1])
    return 1 - dice
